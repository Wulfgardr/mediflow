/* @Codex */
ALTER TABLE `physician_review_attestations` RENAME TO `physician_review_attestations_p2a`;
CREATE TABLE `physician_review_attestations` (
    ...
    CONSTRAINT `physician_review_attestations_timestamp_check` CHECK (typeof(`created_at`) = 'integer' AND `created_at` BETWEEN 0 AND 8640000000000 AND typeof(`updated_at`) = 'integer' AND `updated_at` BETWEEN `created_at` AND 8640000000000 AND (`revoked_at` IS NULL OR (typeof(`revoked_at`) = 'integer' AND `revoked_at` BETWEEN `created_at` AND 8640000000000)))
);
INSERT INTO `physician_review_attestations` SELECT * FROM `physician_review_attestations_p2a`;
DROP TABLE `physician_review_attestations_p2a`;
