...
function subject(replies: readonly Response[], current: (snapshot: unknown) => boolean = () => true) {
    const bodies: unknown[] = []; const requests: Readonly<{ path: unknown; init: RequestInit | undefined }>[] = []; let id = 0;
    const orchestrator = createSmartImportBrowserOrchestrator({ requestId: () => IDS[id++], isCurrent: current,
        fetch: async (path, init) => { requests.push(Object.freeze({ path, init })); bodies.push(JSON.parse(init?.body as string)); const next = replies[bodies.length - 1]; if (!next) throw new Error('unexpected'); return next; } });
    return Object.freeze({ orchestrator, bodies, requests });
}
...
test('posts one exact ingest followed by one exact preview without sending the selection sidecar', async () => {
    ...
    assert.deepEqual(current.requests.map(({ path, init }) => ({ path, method: init?.method, cache: init?.cache, contentType: (init?.headers as unknown as Record<string, string> | undefined)?.['content-type'] })), [
        { path: '/api/ai/smart-import/ingest', method: 'POST', cache: 'no-store', contentType: 'application/json' },
        { path: '/api/ai/smart-import/preview', method: 'POST', cache: 'no-store', contentType: 'application/json' },
    ]);
...
test('classifies every received and unknown POST outcome without retrying', async () => {
    const context = Object.freeze({});
    for (const [replies, code, calls] of [
        [[], 'ingest_outcome_unknown', 1], [[response({}, 500)], 'ingest_unavailable', 1], [[response({ handle: 'bad' })], 'response_invalid', 1],
        [[response({ handle: HANDLE })], 'preview_outcome_unknown', 2], [[response({ handle: HANDLE }), response({}, 503)], 'preview_unavailable', 2], [[response({ handle: HANDLE }), response({ preview: { status: 'bad' } })], 'response_invalid', 2],
    ] as const) {
...
test('rejects raw UUID request identifiers before network work', async () => {
    const context = Object.freeze({}); let fetchCalls = 0; let raw = 0;
