...
test('shares one private H2a owner without exposing lifecycle authority from either facade', () => {
    ...
    const ownerSource = fs.readFileSync(new URL('./headless-soap-active-role-session-grant-production-internal.ts', import.meta.url), 'utf8');
    ...
    assert.equal(ownerSource.match(/\bcreateHeadlessSoapActiveRoleAttestationStore\(\)/gu)?.length, 1);
    assert.equal(ownerSource.match(/\bcreateHeadlessSoapActiveRoleSessionGrantOwner\s*\(/gu)?.length, 1);
    assert.match(ownerSource, /readCurrentSession:\s*readAuthenticatedWebSession/u);
    for (const method of ['withCurrentGrant', 'registerDependent', 'confirmDependent', 'unregisterDependent', 'withCurrentDependent']) {
        assert.match(childSource, new RegExp(`\\b${method}: lifecycle\\.${method}\\b`, 'u'));
    }
    ...
    assert.doesNotMatch(childSource, /patient|selection|digest|approval|proof|entry|writer|fabric|db-server|route/iu);
});
