...
function own(value: unknown, keys: readonly string[]): Record<string, unknown> | null {
  ...
  try {
    const prototype = Object.getPrototypeOf(value); if (prototype !== Object.prototype && prototype !== null) return null;
...
function array(value: unknown): readonly unknown[] | null {
  if (value === null || typeof value !== 'object' || types.isProxy(value) || !Array.isArray(value)) return null;
  try {
    if (Object.getPrototypeOf(value) !== Array.prototype || Object.getOwnPropertySymbols(value).length) return null; const length = Object.getOwnPropertyDescriptor(value, 'length');
    if (!length || !('value' in length) || !Number.isSafeInteger(length.value)) return null; const names = Object.getOwnPropertyNames(value);
    if (names.length !== length.value + 1 || !names.includes('length')) return null;
    for (let index = 0; index < length.value; index += 1) {
      const item = Object.getOwnPropertyDescriptor(value, String(index)); if (!item || !('value' in item) || item.enumerable !== true) return null;
    }
    return value;
  } catch { return null; }
}
...
function safeValue(value: unknown, depth = 0): unknown | null | typeof FORBIDDEN_INPUT {
  ...
  if (value === null || (valueType !== 'string' && valueType !== 'boolean' && valueType !== 'number' && valueType !== 'object')) return null;
  if (valueType === 'object' && types.isProxy(value)) return null;
  if (depth > 8) return null;
  if (valueType === 'string' || valueType === 'boolean') return value;
  if (valueType === 'number') return Number.isFinite(value) ? value : null;
  const itemsValue = array(value);
  if (itemsValue) {
    if (itemsValue.length > 64) return null; const items: unknown[] = [];
    for (let index = 0; index < itemsValue.length; index += 1) {
      const item = safeValue(itemsValue[index], depth + 1); if (item === FORBIDDEN_INPUT) return FORBIDDEN_INPUT; if (item === null) return null;
      items.push(item);
    }
    return frozen(items);
  }
  ...
  try {
    const record = own(value, Object.getOwnPropertyNames(value)); if (!record) return null;
    const copy: Record<string, unknown> = Object.create(null);
    for (const key of Object.keys(record)) {
      if (FORBIDDEN_KEYS.has(key.toLowerCase())) return FORBIDDEN_INPUT; const item = safeValue(record[key], depth + 1); if (item === FORBIDDEN_INPUT) return FORBIDDEN_INPUT; if (item === null) return null;
      copy[key] = item;
    }
...
function normalizePlan(value: unknown): Plan | null | typeof FORBIDDEN_INPUT {
  ...
  for (let index = 0; index < rawActions.length; index += 1) {
    const item = rawActions[index]; const action = own(item, ['actionRef', 'operationId', 'capabilityId', 'applicationServiceRef', 'stage', 'idempotencyKey', 'input']);
...
function normalizeSession(value: unknown): Session | null {
  ...
  for (let index = 0; index < rawCapabilities.length; index += 1) { const capability = text(rawCapabilities[index]); if (!capability) return null; capabilities.push(capability); }
  return frozen({ sessionRef: record.sessionRef as string, active: record.active as boolean, activeRole: record.activeRole as string, leaseEpoch: record.leaseEpoch as number, revoked: record.revoked as boolean, authorizedCapabilityIds: frozen(capabilities) });
}
...
export function createSemanticRuntime(registry: unknown, options: Readonly<{ maxOperations?: number }> = {}): SemanticRuntime {
  ...
  if (!registryInvalid) for (let index = 0; index < entries.length; index += 1) {
