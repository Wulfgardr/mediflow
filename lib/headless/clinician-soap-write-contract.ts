...
const ownKeys = Reflect.ownKeys; const descriptorsOf = Object.getOwnPropertyDescriptors; const prototypeOf = Object.getPrototypeOf;
...
function input(value: unknown): Input | null {
    try {
        ...
        for (const key of KEYS) { const descriptor = descriptors[key]; if (!descriptor || !apply(hasOwn, Object, [descriptor, 'value']) || descriptor.enumerable !== true || typeof descriptor.value !== 'string') return null; copy[key] = descriptor.value; }
        return copy;
    } catch { return null; }
}
...
function digest(fields: readonly string[]): Readonly<{ bytes: readonly number[]; hex: string }> {
    const hash = apply(newHash, undefined, ['sha256']) as typeof hashPrototype; for (let index = 0; index < fields.length; index += 1) { const field = bytes(fields[index]!); apply(hashUpdate, hash, [u32(field.byteLength)]); apply(hashUpdate, hash, [field]); }
    const source = apply(hashDigest, hash, []) as Uint8Array; const raw: number[] = []; let hex = '';
    for (let index = 0; index < source.byteLength; index += 1) { const value = source[index]!; raw[index] = value; hex += HEX[value >>> 4]! + HEX[value & 15]!; }
...
export function validateClinicianSoapWriteDraft(value: unknown): ClinicianSoapWriteContract {
    try {
        ...
        for (const section of SECTIONS) { const sectionValue = normalized(raw[section]); if (sectionValue === null) return deny('invalid_content'); const size = bytes(sectionValue).byteLength; if (size > MAX_SECTION_BYTES) return deny('content_limit'); total += size; if (apply(trim, sectionValue, []).length > 0) meaningful = true; sections[section] = sectionValue; }
