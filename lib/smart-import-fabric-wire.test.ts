...
test('snapshots coherent Smart Import receipt and provenance without retaining caller state', () => {
    const raw = receipt(); const parsed = snapshotSmartImportFabricResolutionReceipt(raw); assert.ok(parsed); assert.equal(parsed.provider, 'ollama'); if (parsed.provider !== 'ollama') throw new Error('Expected local receipt');
...
test('rejects closed-shape, model, provider-receipt, egress, and provenance mismatches', () => {
    const parsed = snapshotSmartImportFabricResolutionReceipt(receipt()); assert.ok(parsed); assert.equal(parsed.provider, 'ollama'); if (parsed.provider !== 'ollama') throw new Error('Expected local receipt');
...
test('rejects hostile nested descriptors, symbols, cycles, and arrays without generic JSON handling', () => {
    ...
    const parsed = snapshotSmartImportFabricResolutionReceipt(receipt()); assert.ok(parsed); assert.equal(parsed.provider, 'ollama'); if (parsed.provider !== 'ollama') throw new Error('Expected local receipt'); assert.equal(snapshotSmartImportFabricProvenance(sparse, parsed), null);
