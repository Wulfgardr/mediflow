...
export const ANYDOC_PDF_PAGE_RENDERER_ENGINE_SHA256 = 'b1495c991d841d474b8c8fe0e69449f1303263f08f0b7f3793609a98929ef132' as const;
export const ANYDOC_PDF_PAGE_RENDERER_DPI = 144;
export const ANYDOC_PDF_PAGE_RENDERER_MAX_PAGES = 16;
...
class PageRenderTimeout extends Error { }
...
async function renderPage(engine: RendererEngine, bytes: Buffer): Promise<{ pngBytes: Buffer; width: number; height: number }> {
    let loading: LoadingTask | null = null; let document: PdfDocument | null = null; let page: PdfPage | null = null;
    let task: RenderTask | null = null; let canvas: RasterCanvas | null = null; let timedOut = false;
    const timer = setTimeout(() => { timedOut = true; task?.cancel(); void loading?.destroy().catch(() => undefined); },
        ANYDOC_PDF_PAGE_RENDERER_PAGE_TIMEOUT_MS);
    try {
        ...
        if (timedOut) throw new PageRenderTimeout();
        if (document.numPages !== 1) throw new Error('not_single_page');
        page = await document.getPage(1); const viewport = page.getViewport({ scale: ANYDOC_PDF_PAGE_RENDERER_DPI / 72 });
        ...
        await task.promise; if (timedOut) throw new PageRenderTimeout();
    ...
    } catch (error) { if (timedOut) throw new PageRenderTimeout(); throw error; }
    finally {
        clearTimeout(timer); try { page?.cleanup(); } catch { /* cleanup is best-effort after denial */ }
        try { await document?.cleanup(); } catch { /* cleanup is best-effort after denial */ }
        try { await loading?.destroy(); } catch { /* cleanup is best-effort after denial */ }
