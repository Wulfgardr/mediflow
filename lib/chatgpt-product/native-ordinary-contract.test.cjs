...
test('nine exact guarded native routes agree with the supplemental OpenAPI and local references', () => {
    const spec = JSON.parse(fs.readFileSync(path.join(f.root,'docs/openapi/native-ordinary-v1.json'),'utf8'));
    const operations = ['project','prepare','status','consent','login/start','login/complete','models','generate','cancel'];
    ...
    for (const operation of operations) {
        ...
        assert.deepEqual(Object.keys(spec.paths[endpoint]),operation === 'project' ? ['post','delete'] : [method]);
        if (operation === 'project') {
            assert.equal(spec.paths[endpoint].delete['x-authority-ceiling'],'scoped_local_candidate');
            assert.equal(spec.paths[endpoint].delete['x-native-only'],'macos');
        }
        ...
        if (operation === 'project') assert.match(source,/export async function DELETE\(/);
