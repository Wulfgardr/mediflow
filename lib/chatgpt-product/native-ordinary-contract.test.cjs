...
const valid = () => ({functionId: 'patient_insight', patientId: 'synthetic-patient', ambulatoryId: 'synthetic-ambulatory', patientRevision: 1, input: {}});
test('preparation is exact passive data: four functions and no session/capability/qualification input', () => {
    for (const functionId of ['patient_insight', 'smart_import', 'document_synthesis', 'treatment_reasoning'])
        assert.equal(parseNativeOrdinaryPreparation({...valid(), functionId}).functionId, functionId);
