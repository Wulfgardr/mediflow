...
test('native HTTP prepare passes a bodyless owned request after bounded parsing',async()=>{
 f.replace('lib/ai-providers/fabric/patient-insight-authenticated-preview-production.ts',{
  async acquireAuthenticatedPatientInsightPreview(){return Object.freeze({});},
 });
 f.replace('lib/ai-providers/fabric/patient-insight-authenticated-preview.ts',{
  createPatientInsightPreviewHttpHandler(){return async()=>Response.json({schema:'synthetic.native.prefix.v1',outcome:'closed'});},
 });
 f.setSetting('ai.fabric.chatgptOrdinary',JSON.stringify({...parseOrdinaryStoredSettings(),enabled:true}));
 const token='synthetic-native-http-'+randomBytes(16).toString('hex');
 const tokenHash=pairing.hashNetworkPairedClientToken(token);f.pair({tokenHash});const session=f.issue({},{tokenHash});f.setSessionCookie(session.id);
 const response=await handleNativeOrdinaryHttp(new Request('http://localhost/api/v1/network/ai/chatgpt/ordinary/prepare',{method:'POST',headers:{'Content-Type':'application/json',
  [pairing.NETWORK_PAIRED_CLIENT_ID_HEADER]:f.binding.clientId,[pairing.NETWORK_PAIRED_CLIENT_TOKEN_HEADER]:token},body:JSON.stringify(f.preparation('patient_insight'))}),'prepare');
 assert.equal(response.status,200);assert.equal((await response.json()).outcome,'closed');
 assert.equal(f.counters.provider,0);assert.equal(f.counters.clinicalWrites,0);
});
