...
let events, counts, workerResults, extractionResults, ownedBytes, session, state, afterWorker, tamperWorkerResult, nativeCapture, restoreBuiltinSpawn;
...
beforeEach(t => {
    ...
    /* @Codex — The real runner imported `spawn` before this test fixture installed its observer.
     * Synchronize the builtin ESM binding, then restore it after all child assertions. */
    syncBuiltinESMExports();
    restoreBuiltinSpawn = () => { t.mock.restoreAll(); syncBuiltinESMExports(); };
...
afterEach(async () => {
    try {
        ...
        flow.cancelOrdinaryGeneration(); f.reset();
        restoreBuiltinSpawn?.(); restoreBuiltinSpawn = null;
    }
});
