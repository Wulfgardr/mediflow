...
export type AttachmentWebDeleteOutcome =
    | Readonly<{ status: 'deleted' }>
    | Readonly<{ status: 'conflict' }>
    | Readonly<{ status: 'failed' }>;

const SNAPSHOT_KEYS = ['id', 'documentSourceRef', 'documentRevision', 'documentFreshnessEpoch'] as const;
const outcome = (status: AttachmentWebDeleteOutcome['status']): AttachmentWebDeleteOutcome => Object.freeze({ status });
...
function mapDenied(code: AttachmentCurrentnessCasCode): AttachmentWebDeleteOutcome {
    switch (code) {
        case 'missing': return outcome('deleted');
