...
const PROFILE = Object.freeze({
    schemaVersion: 'mediflow.ai.provider-instance-profile.v2', providerType: 'openai',
    providerInstance: Object.freeze({ instanceRef: INSTANCE_REF, workspaceRef: 'pws_0123456789abcdef0123456789abcdef' }),
    ...
    model: 'gpt-5.4-mini', capabilities: Object.freeze(['document_synthesis']),
...
function factory(fetch: (url: string, init: RequestInit) => Promise<Response>, overrides: Record<string, unknown> = {}) {
    return Object.freeze({ instanceBinding: instanceBinding(), fetch, ...overrides });
}
...
test('richiede un provider instance lifecycle link host-owned prima di creare il transport', () => {
    assert.throws(() => createOpenAIResponsesOfficialHttpsTransport(Object.freeze({
        fetch: async () => new Response('{}', { status: 200 }),
    })), /OpenAI official HTTPS transport rejected/u);
});

test('nega profile, lifecycle, operation, data use ed endpoint non host-owned', () => {
    const fetch = async () => new Response('{}', { status: 200 });
    const configured = transitionProviderLifecycleV2(createAbsentProviderLifecycleV2(), {
        type: 'configure', binding: BINDING,
    });
    const hostileProfile = Object.defineProperty({ ...PROFILE }, 'model', {
        enumerable: true, get() { throw new Error('must not run'); },
    });
    const invalidFactories = [
        factory(fetch, { instanceBinding: instanceBinding(PROFILE, configured) }),
        factory(fetch, { instanceBinding: instanceBinding({ ...PROFILE, capabilities: ['patient_insight'],
            bindings: [{ operation: 'patient_insight', groupRef: 'group.review-only.v1' }] }) }),
        factory(fetch, { instanceBinding: instanceBinding({ ...PROFILE, dataUse: 'redacted_clinical',
            dataUseProfileRef: 'data-use.redacted-clinical.v1' }) }),
        factory(fetch, { instanceBinding: instanceBinding(hostileProfile) }),
        factory(fetch, { endpoint: ['https:', '', 'caller.invalid'].join('/') }),
    ];
    for (const value of invalidFactories) {
        assert.throws(() => createOpenAIResponsesOfficialHttpsTransport(value),
            /OpenAI official HTTPS transport rejected/u);
    }
});
...
test('fissa URL, metodo, redirect e policy fetch senza accettare endpoint caller-supplied', async () => {
    ...
    const transport = createOpenAIResponsesOfficialHttpsTransport(factory(
...
test('nega request ostili, modello e header extra prima di invocare fetch', async () => {
    let calls = 0;
    const transport = createOpenAIResponsesOfficialHttpsTransport(factory(
        async () => { calls += 1; return new Response('{}', { status: 200 }); },
    ));
...
test('limita lo stream di risposta a maxResponseBytes piu un solo byte sentinel', async () => {
    ...
    const transport = createOpenAIResponsesOfficialHttpsTransport(factory(
        async () => new Response(new Uint8Array(maxResponseBytes + 2_048), { status: 200 }),
    ));
...
test('nega abort, risposte non native e failure fetch con errore sanitizzato', async () => {
    ...
    controller.abort();
    const transport = createOpenAIResponsesOfficialHttpsTransport(factory(
        async () => { calls += 1; throw new Error(`vendor failure ${SECRET}`); },
    ));
    ...
    const failing = createOpenAIResponsesOfficialHttpsTransport(factory(
        async () => { calls += 1; throw new Error(`vendor failure ${SECRET}`); },
    ));
    ...
    const malformed = createOpenAIResponsesOfficialHttpsTransport(factory(
        async () => ({ status: 200, body: '{}' } as never),
    ));
    await assert.rejects(malformed(transportRequest() as never), /OpenAI official HTTPS transport rejected/u);
});

test('ritira Authorization su timeout e cancel anche se fetch ignora abort e resta pending', async () => {
    for (const mode of ['timeout', 'cancel'] as const) {
        ...
        const transport = createOpenAIResponsesOfficialHttpsTransport(factory(async (_url, init) => {
            retainedHeaders = init.headers as Headers;
            markStarted?.();
        ...
        const controller = new AbortController();
        let tick = 1_000;
        const pending = executeOpenAIResponsesV2({
            lifecycle: timedLifecycle, evidence: EVIDENCE,
            secretRef: Object.freeze({ scheme: 'env', name: 'OPENAI_API_KEY' }),
        ...
        await assert.rejects(pending, (error: unknown) => error instanceof OpenAIResponsesV2Error
