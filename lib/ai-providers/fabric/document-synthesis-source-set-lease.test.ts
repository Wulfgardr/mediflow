...
function envelope(sourceText = 'Synthetic source.') { const result = parseDocumentSynthesisProviderEnvelope({ content: JSON.stringify({ schemaVersion: 'mediflow.document-synthesis.provider-envelope.v2', output: { schemaVersion: 'mediflow.ai.extract.v1', task: 'document_synthesis', summary: 'Synthetic summary', data: { qualityLevel: 'green', medications: [], diagnoses: [], problemStatements: [], therapyCandidates: [], servicePrescriptions: [] } }, citations: [{ label: 'S1', quote: sourceText }], claims: [{ claimPath: 'summary', labels: ['S1'] }, { claimPath: 'data.qualityLevel', labels: ['S1'] }] }) }); assert.equal(result.status, 'available'); if (result.status !== 'available') throw new Error('synthetic envelope unavailable'); return result.token; }
