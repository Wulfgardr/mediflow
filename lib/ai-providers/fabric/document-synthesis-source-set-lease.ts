...
export function createDocumentSynthesisSourceSetLease(value: unknown): DocumentSynthesisSourceSetLease {
    ...
    const close = () => { if (closed) return; closed = true; port.dispose(); }; const enter = () => { if (closed) return false; if (active) { reentered = true; closed = true; try { port.dispose(); } catch { /* The authenticated port remains closed by this lease. */ } return false; } active = true; reentered = false; return true; }; const leave = () => { active = false; };
