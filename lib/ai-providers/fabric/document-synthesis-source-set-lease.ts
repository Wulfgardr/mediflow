...
export function createDocumentSynthesisSourceSetLease(value: unknown): DocumentSynthesisSourceSetLease {
    ...
    const close = () => { if (closed) return; closed = true; try { port.dispose(); } catch { /* The lease remains closed when the authenticated port throws. */ } }; const enter = () => { if (closed) return false; if (active) { reentered = true; close(); return false; } active = true; reentered = false; return true; }; const leave = () => { active = false; };
    ...
    return ObjectFreeze({
        ...
        dispose() { if (active) reentered = true; close(); },
