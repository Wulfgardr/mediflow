...
function invoke(entry: Entry, resolution: LocalProviderResolution, prompt: string): Promise<Outcome> {
    ...
    let returned: unknown; try { returned = resolution.adapter.chat([{ role: 'user', content: prompt }], entry.controller.signal, 1400, ObjectFreeze({ responseFormat: 'document_synthesis_v2' })); } catch { resolveOutcome(frozen({ kind: 'failure' as const })); return outcome; }
    try {
...
export async function prepareDocumentSynthesisFabricExecution(handoff: unknown): Promise<DocumentSynthesisFabricExecutionPrepareResult> {
    ...
    const settled = await invoke(entry, admission.resolution, input.prompt);
