...
function invoke(entry: Entry, resolution: LocalProviderResolution, prompt: string): Promise<Outcome> {
    ...
    let returned: unknown; try { returned = resolution.adapter.chat([{ role: 'user', content: prompt }], entry.controller.signal, 1400, ObjectFreeze({ responseFormat: 'json' })); } catch { resolveOutcome(frozen({ kind: 'failure' as const })); return outcome; }
    try {
