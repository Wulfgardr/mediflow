...
function hostContext(value: unknown): Readonly<{ receiptReference: string; provenanceReference: string }> | null {
    ...
    const lease = binding && text(binding.leaseRef); const patient = binding && text(binding.patientRef); const receiptRef = receipt && text(receipt.reference); const provenanceRef = provenance && text(provenance.reference); const provenanceReceipt = provenance && text(provenance.receiptRef);
    if (!binding || !receipt || !provenance || !lease || !patient || !receiptRef || !provenanceRef || !provenanceReceipt || !reference.test(lease) || !reference.test(patient) || !reference.test(receiptRef) || !reference.test(provenanceRef) || !reference.test(provenanceReceipt) || !Number.isSafeInteger(binding.selectionEpoch) || binding.selectionEpoch < 1 || receipt.schemaVersion !== 'mediflow.patient-insight.host-receipt.v1' || receipt.capability !== 'patient_insight' || receipt.authority !== 'host_service' || receipt.writesPerformed !== 0 || receipt.applyPolicy !== 'none' || provenance.schemaVersion !== 'mediflow.patient-insight.host-provenance.v1' || provenance.capability !== 'patient_insight' || provenanceReceipt !== receiptRef) return null;
