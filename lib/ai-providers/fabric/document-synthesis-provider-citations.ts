...
function sealedList<T>(value: readonly T[]): readonly T[] {
    const output: T[] = [];
...
export function validateDocumentSynthesisProviderCitations(value: unknown): DocumentSynthesisProviderCitationsResult {
    try {
        ...
        return sealed({ status: 'available' as const, code: null, schemaVersion: DOCUMENT_SYNTHESIS_PROVIDER_CITATIONS_SCHEMA_VERSION, citations: sealedList(citations), ...COMMON }) as DocumentSynthesisProviderCitationsResult;
    } catch { return denied(); }
}
