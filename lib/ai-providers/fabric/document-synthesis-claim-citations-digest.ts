...
function bytes32(value: unknown): readonly number[] | null {
    try {
        if (typeof value !== 'object' || value === null || IsProxy(value) || !ArrayIsArray(value) || ObjectGetPrototypeOf(value) !== ARRAY || !ObjectIsFrozen(value)) return null;
        const length = ObjectGetOwnPropertyDescriptor(value, 'length'); if (!length || !ObjectHasOwn(length, 'value') || length.value !== 32 || length.enumerable || length.configurable || length.writable) return null;
        const found = ReflectOwnKeys(value); if (found.length !== 33 || found[32] !== 'length') return null; const output: number[] = [];
        for (let index = 0; index < 32; index += 1) { const key = ReflectApply(StringConstructor, undefined, [index]) as string; if (found[index] !== key) return null; const descriptor = ObjectGetOwnPropertyDescriptor(value, key); if (!descriptor || !descriptor.enumerable || !ObjectHasOwn(descriptor, 'value') || descriptor.configurable || descriptor.writable || typeof descriptor.value !== 'number' || !NumberIsSafeInteger(descriptor.value) || descriptor.value < 0 || descriptor.value > 255) return null; output[index] = descriptor.value; }
...
export function digestDocumentSynthesisClaimCitations(value: unknown): DocumentSynthesisClaimCitationsDigest | null {
    try {
        const input = u0(value); if (!input) return null; const payload: number[] = []; const domain = unicode(DOMAIN); if (!domain) return null;
