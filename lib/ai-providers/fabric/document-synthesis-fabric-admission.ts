...
export function admitDocumentSynthesisFabric(value: unknown): DocumentSynthesisFabricAdmissionResult {
    ...
    const finalize = (): DocumentSynthesisFabricCommittedMetadata | null => { if (entry.state !== 'handoff_taken' || !entry.executionToken || !entry.prepared) return null; entry.state = 'finalized'; if (!entry.lease.consume(entry.executionToken)) { entry.lease.dispose(); apply(weakMapDelete, admissions, [token]); if (entry.handoff) apply(weakMapDelete, handoffs, [entry.handoff]); return null; } return entry.prepared; };
    ...
    entry.state = 'pending'; entry.admissionToken = token; entry.providerToken = providerToken; entry.lease = lease; entry.leaseToken = leaseToken; entry.executionToken = null; entry.handoff = null; entry.prepared = null; entry.bindingReceipt = null; entry.capability = capability; entry.execution = null;
