...
function resolution(providerToken: DocumentSynthesisProviderBindingToken): FabricResolution | null {
    const provider = resolveDocumentSynthesisProviderBinding(providerToken); if (!provider) return null; const descriptor = FABRIC_CAPABILITY_DESCRIPTORS.document_synthesis;
...
export function admitDocumentSynthesisFabric(value: unknown): DocumentSynthesisFabricAdmissionResult {
    const input = record(value); const providerToken = input?.providerToken; if (!input || typeof providerToken !== 'object' || providerToken === null || IsProxy(providerToken)) return denied('input_invalid'); const resolved = resolution(providerToken as DocumentSynthesisProviderBindingToken); if (!resolved) return denied('binding_invalid');
...
export function resolveDocumentSynthesisFabricExecutionHandoff(handoff: unknown): DocumentSynthesisFabricExecutionAdmission | null {
    try { if (typeof handoff !== 'object' || handoff === null || IsProxy(handoff)) return null; const entry = apply(weakMapGet, handoffs, [handoff]) as Entry | undefined; if (!entry || entry.state !== 'in_flight') return null; entry.state = 'handoff_taken'; apply(weakMapDelete, handoffs, [handoff]); return entry.execution; } catch { return null; }
}
