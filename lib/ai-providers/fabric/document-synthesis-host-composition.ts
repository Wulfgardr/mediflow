...
export function createDocumentSynthesisHostComposition(configuration: unknown): Readonly<{
    ...
    return frozen({
        issue() {
            ...
            const issued = handleOwner.issue(); if (issued.status !== 'issued' || !issued.documentHandle) return frozen({ status: 'denied' as const, code: 'entropy_unavailable' as const });
            try { mapper = createDocumentSynthesisAuthenticatedSourceMap({ document: { handle: issued.documentHandle, revision: captured.document.revision, freshness: captured.document.freshness }, sources: capturedSources.map((source) => ({ sourceId: source.sourceId, sourceRef: source.sourceRef, digestSha256: source.digestSha256 })) }); handle = issued.documentHandle; state = 'issued'; return frozen({ status: 'issued' as const, code: null }); } catch { state = 'poisoned'; return frozen({ status: 'denied' as const, code: 'entropy_unavailable' as const }); }
        },
        consumeAndMap(value) {
            if (state === 'consuming') { state = 'poisoned'; return denied('handle_consumed'); }
