...
- **Sistema di token reale**: variabili `--mf-*` in `app/globals.css`, esposte come utility Tailwind (`text-ink`, `bg-glass`) in `tailwind.config.ts`. Palette semantica clinica già definita (primary, plum, warning, critical, success, muted).
...
- **La legge "vetro non tocca la clinica" è solo parzialmente applicata**: lo stile attivo (`data-ui-style="redesign"`) ha una regola iniziale di `backdrop-filter: none` per le superfici paziente, ma override successivi applicano ancora blur a `.patient-identity-lens`, `.patient-detail-section` e pannelli affini. La rimozione di quegli override è debito esplicito (vedi [03-materiali.md](./03-materiali.md)).
- **Accessibilità cablata nel CSS, ma non interamente esposta**: `[data-ui-reduce-transparency]`, `[data-ui-reduce-motion]`, `prefers-contrast: more` e `prefers-reduced-motion` hanno override dedicati; il controllo web per Reduce Transparency resta da rendere raggiungibile.
...
- **Stati vuoti onesti** dove il passaggio v2 è arrivato.
...
- **Superfici card fuori vocabolario**: diverse componenti hardcodano `border-slate-200/70 bg-white/80` invece dei token `--paper-*`/`--glass-*`.
...
- La revisione UI/UX 2026-07-02 resta il registro storico dei problemi di interazione: card-soup ematici, terapie che non scalano sul polifarmaco e gerarchia della Scheda. Il censimento storico di 23 `confirm()`, 47 `alert()` e 1 `prompt()` non è backlog corrente.
- **Blur clinico residuo**: override successivi di `app/globals.css` riapplicano `blur(24px) saturate(1.08)` a superfici paziente; vanno ritirati per rendere effettiva la legge dei materiali.
...
- **`PairedPatientsWorkspaceView` (3365 righe al controllo corrente) reimplementa a mano il master-detail** (HStack con colonna fissa 360pt) invece di `NavigationSplitView`, e la lista pazienti è `ForEach` in `ScrollView` invece di `List`: si perdono swipe actions, pull-to-refresh, selezione automatica.
...
| Direzione visiva unica su tutte le piattaforme: Lume come destinazione, Vetro Clinico come baseline transitoria | Direzione di prodotto mantenuta nella lane separata | Niente lingue visive per-piattaforma; cambiano struttura e resa idiomatica, non la semantica clinica condivisa. |
...
Questo folder conserva le fondamenta cliniche e la baseline operativa transitoria; la direzione Lume è mantenuta nella lane separata e procede per slice verificabili.
