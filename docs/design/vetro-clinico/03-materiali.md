...
- Web: `blur(18px) saturate(1.04)` su fondo `--glass-bg`, bordo `--glass-border`, luce superiore inset. Già applicato da `data-ui-style="redesign"` a `.mediflow-sidebar-shell`, `.mediflow-mobile-header`, `.glass-command-capsule`.
...
- Web: `.clinical-paper`, `--paper-bg/border/shadow`. La scelta già in produzione di dare `backdrop-filter: none` alle superfici paziente (`app/globals.css` righe 391-398) è la norma, non un'eccezione.
...
2. **Mai vetro su vetro.** Due superfici traslucide sovrapposte annullano la gerarchia e riducono la leggibilità. È una regola di progetto coerente con l'uso gerarchico e misurato dei materiali Apple; se un popover vetro si apre sopra la sidebar vetro, il livello superiore deve restare chiaramente separato.
...
- I valori web restano deliberatamente conservativi: `--glass-bg` a opacità 0.74 in light non va abbassato senza una nuova misura di contrasto sul composito reale.
