...
- **Il dark deriva dai token, mai da letterali.** Oggi `.graphite-chip-tone-*` in dark usa `rgb()` scritti a mano (`app/globals.css` righe ~992-1099): vanno riespressi come funzione dei token (`color-mix()` in CSS o varianti dark esplicite nel JSON sorgente, una sola volta).
