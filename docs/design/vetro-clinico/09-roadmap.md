...
Questo piano non biforca la roadmap in 7 fasi della revisione UI/UX 2026-07-02: la assume come binario dei problemi di interazione e vi aggiunge le corsie del sistema di design (DS). Dove una corsia DS tocca una fase esistente, lo dichiara.
...
- Percorsi principali: `app/globals.css`, `app/layout.tsx`, `tailwind.config.ts`, componenti card e `components/ui/badge.tsx`.
- Gate: smoke visivo su `/mockups/scheda` e cockpit, light+dark+Reduce Motion e Increase Contrast, più Reduce Transparency quando il relativo controllo sarà raggiungibile; zero valori letterali nuovi (grep di verifica su `#`/`rgb(` nei diff).

### DS-2. Consolidamento nativo Apple

Le azioni di [07-piattaforme/apple.md](./07-piattaforme/apple.md) e del contratto Lume macOS mantenuto nella lane separata: un solo sistema di card cliniche opache, `NavigationSplitView`, spacchettamento del workspace, via gli artefatti di rollout, palette semantica unica, guardie di accessibilità e disposizione sidebar/workbench/inspector.
...
ConfirmDialog/Toast come baseline dei flussi di feedback, combobox sui tre autocomplete clinici, `useLiveQueryState` sulle 28 superfici mute. Contributo DS: le specifiche di [04-interazione.md](./04-interazione.md) sono il capitolato di quella fase.
