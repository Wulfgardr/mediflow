...
read_when:
  - "Working on native/MediFlowAppleApp or MediFlowAppleShared UI, or reviewing native design parity."
---

# Piattaforma: Apple (iOS, iPadOS, macOS)

Contratto: outcome = client accoppiato con la stessa grammatica del web, reso con i mezzi nativi. Floor delle app: iOS 17 / macOS 14; il package condiviso dichiara macOS 13 per la compilazione del core. Liquid Glass reale è usato solo su OS 26+ dietro `#available`. UI: SwiftUI, condivisa in `MediFlowAppleShared`, differenziata da size class e `#if os`.
...
2. **`NavigationSplitView` per il workspace pazienti**: sostituisce il master-detail manuale di `PairedPatientsWorkspaceView` (HStack + colonna fissa 360pt). La `List` nella colonna si coordina con la selezione e la split view può dichiarare larghezze minime, ideali e massime; swipe e refresh vanno verificati separatamente per piattaforma.
3. **Spacchettare `PairedPatientsWorkspaceView`** (3350 righe al 2026-07-11) in viste per dominio (lista, quadro paziente, diario, terapie, osservazioni): prerequisito per qualsiasi lavoro di design fine.
...
5. **`Color.red` -> `VetroPalette.tint(for: .critical)`** nel punto noto; nessun colore diretto fuori da `VetroPalette`.
