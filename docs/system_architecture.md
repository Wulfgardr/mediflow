...
3. **Home-base opt-in**: esiste una slice `network-home-base` su
   ...
   e diario clinico/terapie versionati, mentre pairing, replica, altri child CRUD e
   sync restano governati e separati.
...
| `/api/v1/network/*` | paired client credential + sessione operatore | `home-base` read-only-first + primi write limitati paziente/diario/terapie versionati |
