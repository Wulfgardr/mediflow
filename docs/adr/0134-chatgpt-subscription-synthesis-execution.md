...
## Contratto Mac ordinario — candidato locale 3f2e6f2a (18 settembre 2026)

Questo delta precede la relativa implementazione. Il risultato unitario comprende
Patient Insight, Smart Import, Document Synthesis e Treatment Reasoning. Il canale
Web conserva il proprio issuer; un native non viene convertito in WebSessionProjection.

Il package lifecycle emette port nativi opachi solo dalla sessione nativa originale,
legati alla generazione, al dispositivo Mac e al token hash ammesso al login. La
scadenza è quella originaria, con limite monotono: i successivi accessi non rinnovano
il port. Il relativo adapter concreto interseca questa autorità con il pairing,
la modalità home-base, il ruolo operatore corrente e le capability già concesse
`network.ai.central-runtime` e `network.replica.readonly-patients`. `native.ai.configure`
non è un permesso d'inferenza. Non si accettano un grant dal corpo HTTP né un
callback che possa attestare arbitrariamente la validità. Le letture di autorizzazione
sono fisse; una revoca osservata brucia la generazione nativa prima del cleanup.

Il nucleo comune riguarda esclusivamente operazioni lifetime equivalenti: mint,
begin/commit/abort use, binding sincrono e cleanup. Gli issuer Web e nativo, i loro
registri e le rispettive prove restano distinti. La composizione Mac usa gli stessi
Application Services, builder, parser e commit delle quattro funzioni. I DTO di
contesto sono dati per le acquisizioni canoniche, mai port o capacità; patientId,
ambulatorio, revisione e handle devono essere riconfermati dagli owner originali.
Le route native sono adapter nominati, senza query DB o applicazione clinica.

La preparazione seleziona il contesto localmente, senza aprire l'account. La sequenza
è: preparazione/redazione e congelamento UTF-8 → consenso sulla revisione precisa →
login dedicato → catalogo del processo → opzione modello/effort → turno → parser e
commit proposal-only della funzione originaria → cleanup verificato → pubblicazione.
Il medesimo oggetto di consenso resta necessario fino al commit. Account informativo,
preferenza salvata o selezione da soli non autorizzano esecuzione. Errori, revoche,
cambio paziente/revisione, clone, risultato tardivo o cleanup incerto negano la
pubblicazione e non selezionano un provider alternativo.

Le nuove route `/api/v1/network/ai/chatgpt/ordinary/*` sono descritte nel supplemento
OpenAPI `docs/openapi/native-ordinary-v1.json`. Il supplemento è additivo: lo snapshot
non contiene il file canonico mediflow-v1.yaml, che non viene ricostruito.
L'override documentale Mac è circoscritto dall'addendum ADR0076 in questo diff.

Restano intatti C2, custode, sandbox, drain, asset, pin OS/processo, DB/crypto, OCR,
