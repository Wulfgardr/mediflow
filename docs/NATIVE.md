...
> [!IMPORTANT]
> Il congelamento dello snapshot macOS precedente a `v0.4.0` è storico. La base
> attiva è oggi l'app universale descritta da ADR 0048/0071: bundle macOS
> `home-base`, target iPhone/iPad paired e package condiviso
> `MediFlowCore`/`MediFlowAppleShared`. La web app resta la superficie più
...
La base corrente va letta così:

* **macOS**: `MediFlowMacApp` è il fronte più maturo. Il bundle packaged include
  il WebRuntime Next standalone e può osservare/gestire esplicitamente backend
  locale e proxy TLS senza diventare supervisore di Ollama o Docker.
...
* **Core condiviso**: `MediFlowCore` concentra contratti, cifratura, codec,
  ...
  portabilità del core, non app desktop complete su tre sistemi.
...
* **Parity**: la matrice post-Wave 5 è in [docs/parity-matrix.md](./parity-matrix.md).
  WUL-401 copre la click-map macOS P6; WUL-403 resta la corsia per rendere
  visibili età/TTL/staleness della cache e il degrado offline read-only.
...
restano host-only o review-only quando ADR 0073/0076 lo richiedono.

### 3. Design e accessibilità

Il kit [Vetro Clinico](./design/vetro-clinico/README.md) condivide grammatica e
toni, ma adatta struttura e input a macOS, iPadOS e iOS. Liquid Glass è usato
solo su OS 26+ con fallback; la documentazione del design non equivale a una
certificazione VoiceOver o a parity UI completa.
...
3. Mantieni sigillo/decrittazione nel boundary client già esistente: non creare
