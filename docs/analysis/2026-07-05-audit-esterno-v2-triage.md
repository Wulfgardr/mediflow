...
## Finding aggiuntivo (nostro, non dell'audit)

La recon ha rilevato un drift documentale: il write-path di rete (`network.replica.write-clinical-diary` in `lib/network-entry-write.ts`, montato su `app/api/v1/network/patients/[id]/entries/route.ts` POST) permette gia' ai client paired di creare/aggiornare voci di diario clinico da remoto, che e' oltre la "read-only first slice" descritta in ADR 0034/0035. Nessun ADR piu' recente formalizza questa capability. Non contraddice il modello single-writer/node-autoritativo, ma gli ADR come scritti descrivono uno slice piu' stretto di cio' che il codice implementa. Da promuovere/superare gli ADR o documentare la capability.
