...
| Estrazione locale unica degli allegati con AnyDoc | [docs/adr/0107-anydoc-local-attachment-extraction.md](./adr/0107-anydoc-local-attachment-extraction.md) | `CANONICAL / ACCEPTED` | Prevale sulle clausole OCR incompatibili: AnyDoc e estrazione deterministica, non OCR; `ocr` resta classificata ma terminalmente `unavailable`, senza venue, provider, fallback o kill switch. |
...
| Parity operativa MLX benchmark-visible | [docs/mlx-operational-parity.md](./mlx-operational-parity.md) | `SECONDARY` | Fissa `WUL-165`: MLX e visibile in benchmark e diagnostica read-only, ma resta fuori dal runtime clinico; Ollama resta il default generativo. |
