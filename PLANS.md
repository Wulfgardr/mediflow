...
- [ ] Formalizzare e avviare il rollout Apple-native condiviso (`WUL-187`): ADR architettura shared Apple client (`WUL-188`), hardening trasporto paired (`WUL-189`), boundary write remoto reviewable (`WUL-190`), target shell condivisi (`WUL-191`), macOS `home-base` packaged (`WUL-192`) e client iPhone/iPad paired con parity non-AI + cache locale (`WUL-193`, `WUL-194`). Primo slice `WUL-192`: shell Apple/home-base come entrypoint macOS primario e pannello runtime read-only; la supervisione automatica dei processi resta aperta.
...
- `WUL-192` e in corso come slice esplicitamente progressiva: il bundle macOS mostra prima il shell Apple/home-base e il readiness runtime locale; non dichiara ancora "no terminal" completo finche backend/TLS/Ollama/Docker non hanno supervisione app-managed.
