...
- `WUL-192` e in corso come slice esplicitamente progressiva: il bundle macOS mostra il shell Apple/home-base, il readiness runtime locale e puo gestire esplicitamente il proxy TLS; il backend standalone deve passare `check:standalone-runtime-bundle` prima di essere copiato nella app e non si dichiara ancora "no terminal" completo finche backend/Ollama/Docker non hanno supervisione app-managed.
