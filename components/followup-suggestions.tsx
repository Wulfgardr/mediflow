                ...
                >
                    <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                            <p className="text-sm font-medium text-[color:var(--mf-ink)]">{suggestion.label}</p>
                            {suggestion.excerpt ? (
                                <p className="mt-1 text-xs leading-5 text-[color:var(--mf-muted)]">
                                    <PrivacyBlur intensity="sm">{suggestion.excerpt}</PrivacyBlur>
                                </p>
                            ) : null}
                            <p className="mt-1.5 text-[11px] text-[color:var(--mf-muted)]">
                        ...
                        {openId !== suggestion.id ? (
                            <button
                                ...
                                className="inline-flex shrink-0 items-center gap-1.5 rounded-full border border-[color:rgba(112,106,100,0.2)] px-3 py-1.5 text-xs font-medium text-[color:var(--mf-ink)] transition-colors hover:bg-white/60 dark:border-[color:rgba(255,247,240,0.14)] dark:hover:bg-white/5"
                            >
                    ...
                    {openId === suggestion.id ? (
                        <div className="mt-3 space-y-2 border-t border-[color:rgba(112,106,100,0.12)] pt-3">
                            <label className="block text-[11px] font-medium text-[color:var(--mf-muted)]">
                                Titolo
                                <input
                            ...
                            {error ? <p className="text-xs text-[color:var(--mf-critical)]">{error}</p> : null}
                            <div className="flex items-center gap-2 pt-1">
                                <button
                                    type="button"
                                    onClick={confirm}
                                    disabled={saving}
                                    className="rounded-full bg-[color:var(--mf-ink)] px-4 py-1.5 text-xs font-semibold text-white disabled:opacity-50"
                                >
                                ...
                                <button
                                    ...
                                    disabled={saving}
                                    className="rounded-full px-3 py-1.5 text-xs font-medium text-[color:var(--mf-muted)] hover:text-[color:var(--mf-ink)]"
                                >
                                    Annulla
                                </button>
                            </div>
