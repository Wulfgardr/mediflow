...
- L'app Next.js fornisce:
  ...
  - contratto versionato `/api/v1/*`, inclusa la slice `network` paired con read pazienti, write profilo/status e primi read/write diario clinico e terapie
...
profilo/status paziente, diario clinico e terapie per client trusted su LAN.
...
- **Network API** (`/api/v1/network/*`):
  ...
  - resta read-only-first, con write limitati a profilo/status paziente, diario clinico e terapie versionate
...
- `local-only` come default e `network-home-base` come opt-in paired/read-only-first con write paziente, diario e terapie limitati/versionati.
