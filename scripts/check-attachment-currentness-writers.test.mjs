...
test('exact repository inventory is classified and keeps known gaps red', () => {
    ...
    assert.deepEqual([...new Set(findings.map((item) => item.code))].sort(), [
        'AUTHENTICATED_METADATA_PUT_CURRENTNESS_GAP',
    ...
    assert.deepEqual(policyFindings().map((item) => item.code), ['BACKUP_LEGACY_CURRENTNESS_GAP', 'RESTORE_PREFLIGHT_CURRENTNESS_GAP', 'DELETE_LOCATOR_REVOCATION_GAP', 'PURGE_LOCATOR_REVOCATION_GAP', 'RESTORE_LOCATOR_REVOCATION_GAP']);
});
