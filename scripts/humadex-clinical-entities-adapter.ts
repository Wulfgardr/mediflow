...
const DEFAULT_PYTHON_PATH = path.join(PROJECT_ROOT, '.venv_humadex', 'bin', 'python');
const RUNNER_PATH = path.join(__dirname, 'humadex-clinical-entities-runner.py');
const STARTUP_TIMEOUT_MS = Number.parseInt(process.env.MEDIFLOW_HUMADEX_STARTUP_TIMEOUT_MS || '300000', 10);
const REQUEST_TIMEOUT_MS = Number.parseInt(process.env.MEDIFLOW_HUMADEX_REQUEST_TIMEOUT_MS || '120000', 10);
const CONFIDENCE_THRESHOLD = Number.parseFloat(process.env.MEDIFLOW_HUMADEX_CONFIDENCE_THRESHOLD || '0');
const CLOSE_TIMEOUT_MS = Number.parseInt(process.env.MEDIFLOW_HUMADEX_CLOSE_TIMEOUT_MS || '2000', 10);

let runnerPromise: Promise<RunnerClient> | null = null;

function resolvePythonExecutable() {
    if (process.env.MEDIFLOW_HUMADEX_PYTHON) return process.env.MEDIFLOW_HUMADEX_PYTHON;
...
class RunnerClient {
        ...
        this.ready = new Promise<RunnerReadyMessage>((resolve, reject) => {
            const startupTimeout = setTimeout(() => {
                reject(new Error(formatRunnerFailure(
                    `HUMADEX runner did not become ready within ${STARTUP_TIMEOUT_MS}ms.`,
                    this.stderrLines,
                )));
            }, STARTUP_TIMEOUT_MS);
            ...
            this.child.once('exit', (code, signal) => {
                ...
                const error = new Error(formatRunnerFailure(
                    `HUMADEX runner exited before readiness (code=${code ?? 'null'}, signal=${signal ?? 'null'}).`,
                    this.stderrLines,
                ));
                reject(error);
    ...
    async analyze(text: string) {
        ...
        const id = `humadex-${++this.nextId}`;
        const payload = JSON.stringify({
            id,
            text,
            confidenceThreshold: Number.isFinite(CONFIDENCE_THRESHOLD) ? CONFIDENCE_THRESHOLD : 0,
        });

        return new Promise<unknown>((resolve, reject) => {
            const timeout = setTimeout(() => {
                this.pending.delete(id);
                reject(new Error(`HUMADEX runner timed out after ${REQUEST_TIMEOUT_MS}ms.`));
            }, REQUEST_TIMEOUT_MS);
...
export function createAdapter() {
    return {
        name: 'humadex_italian_medical_ner',
