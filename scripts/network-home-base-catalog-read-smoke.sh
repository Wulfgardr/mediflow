...
DATA_DIR="${MEDIFLOW_NETWORK_SMOKE_DATA_DIR:-$ROOT_DIR/tmp-network-home-base-catalog-read/$(date +%Y%m%d-%H%M%S)}"
LOG_DIR="$DATA_DIR/logs"
DEV_LOG="$LOG_DIR/next-dev.log"
BASE_URL="${E2E_BASE_URL:-http://127.0.0.1:3200}"
REPORT_PATH="$DATA_DIR/reports/network-home-base-catalog-read-report.json"
...
echo "Running network home-base catalog read smoke..."
node --test --test-concurrency=1 scripts/network-home-base-catalog-read.test.mjs

echo "Network home-base catalog read smoke completed."
