...
function Fixture(){const[active,setActive]=useState(true);const[context,setContext]=useState(1);const[reads,setReads]=useState(0);return <main><h1>Preferenze AI · fixture sintetica</h1><FunctionPreferencesContent active={active} onRead={()=>setReads(n=>n+1)}/><output data-testid="read-count">{reads}</output><Preview context={context} active={active}/><button id="context" onClick={()=>setContext(x=>x+1)}>Cambia contesto</button><button id="lock" onClick={()=>{window.fixtureLocked=true;setActive(false)}}>Blocca fixture</button></main>};createRoot(document.getElementById('root')).render(<Fixture/>);`);
...
try {
 ...
 for(const width of [390,965,1440]){
  ...
  assert.equal(await page.locator('[data-testid="function-preferences"] article').count(),4);await page.waitForFunction(()=>document.querySelector('[data-testid=read-count]')?.textContent==='1');await page.getByRole('button',{name:'Rileggi impostazioni',exact:true}).click();await page.getByRole('button',{name:'Spegni tutte',exact:true}).waitFor();assert.equal(await page.getByTestId('read-count').innerText(),'1');
  assert.equal(calls.some(c=>c.path==='/api/settings/ai/chatgpt/models'),false);
  const geometry=await page.evaluate(()=>({width:innerWidth,scroll:document.documentElement.scrollWidth,cards:[...document.querySelectorAll('article')].map(e=>({radius:getComputedStyle(e).borderRadius,width:e.getBoundingClientRect().width})),targets:[...document.querySelectorAll('[data-testid="function-preferences"] button,[data-testid="function-preferences"] select')].filter(e=>e.checkVisibility()).map(e=>e.getBoundingClientRect().height)}));
 ...
 await page.getByRole('button',{name:'Ripristina predefiniti'}).click();await proposal.waitFor();conflict=true;await page.getByRole('button',{name:'Applica alle impostazioni'}).click();await page.getByRole('alert').filter({hasText:/decidi di nuovo/}).waitFor();assert.equal(await page.getByText('Impostazioni salvate e rilette. Nessuna modifica clinica.').count(),0);conflict=false;
 state=initial();await page.goto(origin);const picker=page.locator('#preview');assert.equal(await picker.locator('select').count(),1);await picker.getByLabel('Modello per questa proposta',{exact:true}).focus();await picker.locator('option').filter({hasText:'synthetic-local:large'}).waitFor({state:'attached',timeout:3000}).catch(async error=>{console.log(await picker.innerText());console.log(errors);throw error});await picker.getByLabel('Modello per questa proposta',{exact:true}).selectOption(o('b'));assert.equal(await picker.getByRole('button',{name:'Rileggi modelli'}).count(),0);for(const width of [390,965,1440]) {await page.setViewportSize({width,height:1000});const compact=await picker.getByTestId('model-picker-patient_insight').evaluate(el=>({height:el.getBoundingClientRect().height,overflow:document.documentElement.scrollWidth>innerWidth,paragraphs:[...el.querySelectorAll('p')].filter(p=>p.checkVisibility()).length,controls:[...el.querySelectorAll('select,button,summary')].filter(e=>e.checkVisibility()).map(e=>e.getBoundingClientRect().height)}));assert.equal(compact.overflow,false);assert.equal(compact.paragraphs,0);assert.ok(compact.height<=104);assert.ok(compact.controls.every(h=>h>=44));results.push({width,compact});await picker.screenshot({path:join(output,`picker-${width}.png`)});}await picker.getByRole('button',{name:'Genera fixture'}).click();await picker.getByText('Proposta sintetica ricevuta').waitFor();
 ...
 await page.locator('#context').click();assert.equal(await picker.locator('select').inputValue(),'');
 await picker.getByLabel('Modello per questa proposta',{exact:true}).focus();await picker.locator('option').filter({hasText:'synthetic-local:large'}).waitFor({state:'attached',timeout:3000}).catch(async error=>{console.log(await picker.innerText());console.log(errors);throw error});await picker.getByLabel('Modello per questa proposta',{exact:true}).selectOption(o('b'));state.catalogRevision=r('e');await picker.getByRole('button',{name:'Genera fixture'}).click();await picker.getByRole('alert').filter({hasText:/nessun modello alternativo/}).waitFor();
 assert.equal(calls.filter(c=>c.path==='/api/ai/patient-insight/preview').length,1);
 await picker.getByRole('button',{name:'Rileggi modelli'}).click();await picker.getByRole('button',{name:'Riusa predefinito'}).waitFor();assert.equal(await picker.getByRole('button',{name:'Genera fixture'}).isDisabled(),true);await picker.getByRole('button',{name:'Riusa predefinito'}).click();assert.equal(await picker.getByRole('button',{name:'Genera fixture'}).isDisabled(),false);
 ...
 state=initial();state.functions[0].options=[];state.functions[0].bindingState='unsupported';state.functions[1].options.forEach(o=>o.state='unavailable');await page.goto(origin);await page.getByText('Predefinito non disponibile.',{exact:true}).waitFor();await page.screenshot({path:join(output,'unavailable.png'),fullPage:true});
 ...
 console.log('PASS: stable onRead with unstable callback; compact picker; 3 widths, keyboard, 44px, 12px, preview/apply/CAS, picker header/currentness/lock; synthetic only.');
